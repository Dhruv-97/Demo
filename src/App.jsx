...
function GenuinPlacement({ viewStyle }) {
  useEffect(() => {
    const initializeGenuin = () => {
      const container = document.getElementById("gen-sdk");
  ...
  return (
    <section
      className={`genuin-card genuin-card--${viewStyle}`}
      aria-labelledby={`genuin-heading-${viewStyle}`}
    >
      <div>
        <p className="eyebrow">Community picks</p>
        <h2 id={`genuin-heading-${viewStyle}`}>See what people are craving</h2>
      </div>
      <div id="gen-sdk" className="gen-sdk-class" />
    </section>
  );
}
  ...
  return (
    <div className="page">
      <header className="topbar">
        <a className="brand" href="/" aria-label="BiteRun home">
          BiteRun
        </a>
        <label className="search" aria-label="Search restaurants or foods">
        ...
        <div className="view-switch" aria-label="Placement view style">
      ...
      <main className="marketplace">
        <aside className="filters" aria-label="Delivery filters">
          <h2>Filters</h2>
          <div className="toggle-group" aria-label="Order type">
            <button className="active">Delivery</button>
            <button>Pickup</button>
          </div>
          <p className="arrival">Arrives today, ASAP</p>
          {filters.map((filter) => (
            <label key={filter} className="check-row">
              <input type="checkbox" />
              <span>{filter}</span>
            </label>
          ))}
        ...
        <section className="content" id="restaurants">
          ...
          <section aria-labelledby="browse-heading">
            ...
            <div className="cuisine-grid">
              {cuisines.map((cuisine) => (
                <article className="cuisine-card" key={cuisine.name}>
        ...
        <aside className="side-rail" id="offers" aria-label="Trending offers">
          {placementView === "rail" && (
